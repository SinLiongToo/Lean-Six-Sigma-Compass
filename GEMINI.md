...
2. **Interactive Event & Global Scope Integrity (Button/Action Clickability)**:
   - When an HTML element uses an inline handler (`onclick="foo(...)"`, `onchange="..."`, etc.), `foo` MUST be accessible in the browser's global scope (`window`).
...
## 🔬 Cleanroom 5S / 6S Zero-Contamination Architecture & Protocol
In semiconductor fabs, advanced packaging (CoWoS), and EUV optical assembly:
