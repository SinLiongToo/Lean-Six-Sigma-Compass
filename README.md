...
[![Version](https://img.shields.io/badge/Version-v2.66-brightgreen?style=flat-square)](#)
...
Masa Tu 精實六標準差羅盤 (共 24 大核心模組)
 ...
 ├── 23. 相關推薦書籍 (48 本跨領域經典精實書庫 & Obsidian 風格知識關聯網絡圖)
...
### 9. 📚 相關推薦書籍 (48 本經典跨領域書庫 & Obsidian 風格知識網絡圖)
...
- **Obsidian 雙鏈筆記風格星空關聯網絡 (Zero-Dependency Interactive Canvas Graph)**：
  ...
  - **節點拖曳固定與復位全解鎖 (Drag-to-Pin Anchor & Reset Unpin)**：
    - **手動拉拖即固定 (Drag to Pin)**：滑鼠或手指拖曳任一節點（包含 7 大領域樞紐或 48 本書籍節點）至指定位置，移動超過 4px 放開即刻鎖定座標（`isPinned = true`），作為自訂物理力學錨點（仍對周邊節點產生斥力/拉力，但自身座標不再漂移，即使點擊「⚡ Simulate / 物理重排」也不會被震開）。
    - **多重視覺標記 (Visual Indicators)**：已固定節點外圍顯示亮眼琥珀色虛線定位光環與右上角圖釘小圓標，標籤文字前綴 `📌` 圖示並常駐顯示；浮動檢視卡片（Inspector）同步呈現 `📌 已固定位置 (點擊解鎖)` / `📍 未固定 (點擊固定)` 動態晶片。
    - **靈活解鎖與全域復位 (Unpin & Reset Options)**：支援點擊檢視卡片晶片解鎖、畫布雙擊（Double Click）任一節點即時切換固定狀態、或點擊「⤢ 視野復位 / Reset」一鍵全域解鎖所有節點並重設鏡頭。
...
- 精選與精實六標準差、心智模型、數據檢驗、專注力、敏捷試錯、迎變世代與生成式 AI 深度結合的 48 本跨界經典著作：
  - 《深度數位大掃除》、《沒有Email的世界》、《迎變世代》、《專注力協定》、《別自個兒用餐》、《投資最重要的事》、《職場人的生成式AI工作法》、《寫下來，事情就會發生》、《當下的力量》、《我可能錯了》、《5秒法則》、《做自己的人生財務長》、《跨能致勝》、《給予》、《真確》、《刻意練習》、《精實創業》、《豐田模式》、《目標：限制理論》、《原子習慣》、《致富心態》、《深度工作力》、《窮查理的普通常識》、《聰明犯錯》、《卡內基溝通與人際關係》、《當我談跑步時我談些什麼》、《垃圾車法則》、《活出意義來》、《與成功有約》等。
...
- **5S / 6S (EHS) 現場稽核與無塵室零污染防線系統**：
  ...
  - 嚴格界定無塵室四大紅線：① 身上禁物與手汗防護（禁化妝品/香水等 VOC 來源、進室洗手烘乾並配戴吸汗內襯手套防手汗滲漏、禁普通紙筆、禁手機飾品硬幣、開箱室脫除外箱瓦楞紙）；② 阻絕 VOC、氯污染與 AMC 分子污染（全面落實無鹵素 Halogen-Free、嚴禁 PVC 膠帶與含氯洗劑/漂白水以杜絕光罩/晶圓產生 NH₄Cl 氯化銨白霧結晶與銅點蝕、密封件高溫除氣、無氯超純水 UPW 與電子級 IPA 單向擦拭）；③ FOD 異物掉落防呆（機台與晶圓上方 100% 工具繫繩 Tool Tethering、無塵衣口袋淨空、維修前後零組件 100% 清點對齊防晶圓碎裂）；④ 嚴禁隨意觸碰防線（晶圓表面/FOUP內部/光罩/腔壁絕對零徒手碰觸以阻絕手汗中致命 NaCl 氯離子金屬腐蝕與鈉離子穿透氧化層、嚴禁倚靠機台外殼、真空吸筆專用移載、定點更換無塵丁腈手套）。
